<div class="d-flex flex-column align-items-center justify-content-center" style="width: 100%;position: relative;">
    <button style="border: none;background-color: transparent;" onclick="window.openWindow('generalStoreContainer')">
        <img src="/images/close.png" style="position: absolute;right: 2px;top: 1px; width: 25px;z-index: 999">
    </button>
    <p style="font-size: smaller;position: absolute;z-index: 999;color: white;top:5px;">{{__('custom.navbar_shop_general')}}</p>
                    ...
                    @if($shopItem->count > 0)
                        @php
                            ...
                            elseif(isset($item) and \App\Http\Controllers\Controller::getType($item) === 'body'){
                                ...
                                $title['name']=$name;
                                $title['yang']=$shopItem->price.' Yang';
                                $title['potion']=1;
                            }
                        @endphp
                        ...
                        @else
                            <button class="tooltipItem"
                                    style="width:32px;
                                    height: <?=$item->size*32?>px;"
                                    data-bs-toggle="modal" data-bs-target="#buyModalGeneral"
                                    onclick="document.getElementById('shopIndexGeneral').value='{{$shopItem->id}}';document.getElementById('buyModalGeneralBody').innerText= '{{__('custom.want_to_buy',['count'=> $shopItem->count, 'name'=> \App\Http\Controllers\Controller::getType(\App\Models\Item::where('vnum',$shopItem->itemId)->first())=='potion'?__('items.item_id_'.$shopItem->itemId):__('items.item_id_' . substr_replace($shopItem->itemId, '', -1)) . substr($shopItem->itemId, -1), 'price'=> $shopItem->price] ) }}'">
                                <img
                                        src="/images/items/{{str_pad($shopItem->itemId,5,'0',STR_PAD_LEFT)}}.png"
                                        width="32" height="{$item->size*32}"
                                        data-file-width="32"
                        ...
                        @endif
                                <div style="pointer-events:none;">
                                    @if(is_array($title) && $title['potion']==0)
                                        <p>
                                    ...
                                    @else
                                        <p>
                                            <span style="color:{{$title['name'][1]}}">{{$title['name']}}</span>
                                        </p>
                                        <p>
                                            <span style="color:<?php if($shopItem->price > auth()->user()->player->gold){echo "red";}else{echo "green";}?>">{{$title['yang']}}</span>
                                        </p>
                                    @endif
                                </div>
                            </button>
...
<div class="modal fade" id="buyModalGeneral" tabindex="-1" aria-labelledby="buyModalGeneralLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body" id="buyModalGeneralBody"></div>
            <div class="modal-footer justify-content-center justify-content-evenly">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <input name="shopIndex" id="shopIndexGeneral" type="hidden" value="">
                <button type="submit" class="btn btn-success" data-bs-dismiss="modal"
                        onclick="buyItem(document.getElementById('shopIndexGeneral').value)">buy
                </button>
            </div>
        </div>
    </div>
