<div class="d-flex flex-column align-items-center justify-content-center" style="width: 100%;position: relative;">
    <button style="border: none;background-color: transparent;" onclick="window.openWindow('weaponStoreContainer')">
        <img src="/images/close.png" style="position: absolute;right: 2px;top: 1px; width: 25px;z-index: 999">
    </button>
    <p style="font-size: smaller;position: absolute;z-index: 999;color: white;top:5px;">{{__('custom.navbar_shop_weapon')}}</p>
                    ...
                    @if($shopItem->count > 0)
                        ...
                        @else
                            <button type="submit"
                                    ...
                                    style="width:32px;
                                        height: <?=$item->size*32?>px;"
                                    data-bs-toggle="modal" data-bs-target="#buyModalWeapon"
                                    onclick="document.getElementById('shopIndexWeapon').value='{{$shopItem->id}}';document.getElementById('buyModalWeaponBody').innerText= '{{__('custom.want_to_buy',['count'=> $shopItem->count, 'name'=> \App\Http\Controllers\Controller::getType(\App\Models\Item::where('vnum',$shopItem->itemId)->first())=='potion'?__('items.item_id_'.$shopItem->itemId):__('items.item_id_' . substr_replace($shopItem->itemId, '', -1)) . substr($shopItem->itemId, -1), 'price'=> $shopItem->price] ) }}'"
                            >
                                <img
                                        src="/images/items/{{str_pad($shopItem->itemId,5,'0',STR_PAD_LEFT)}}.png"
                                        width="32" height="{$item->size*32}"
...
<div class="modal fade" id="buyModalWeapon" tabindex="-1" aria-labelledby="buyModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body" id="buyModalWeaponBody"></div>
            <div class="modal-footer justify-content-center justify-content-evenly">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <input name="shopIndex" id="shopIndexWeapon" type="hidden" value="">
                <button type="submit" class="btn btn-success" data-bs-dismiss="modal" onclick="buyItem(document.getElementById('shopIndexWeapon').value)">buy</button>
            </div>
        </div>
    </div>
</div>
